...
The evolution path is:
	•	v1 — Basic merger
	•	v2 — Portfolio fusion
	•	v3 — Real-time variant tracking

For interoperability, Unispec outputs are designed to feed into:
	•	Figma design systems
	•	OpenAI GPT context
	•	VSCode architecture diagrams
...
⚠️ LLM specs accurate — Core assumption with partial validation.
If wrong: Product becomes unreliable specification compiler.

Suggested validation: Manual audit of 20 extracted specs against human-written docs.
...
Authentication Model
	•	OpenAI LLM Access: API key via environment variable
	•	Local Processing: App analysis client-side
	•	No Persistent Storage: Temporary in-memory processing only

Environment Variables

Variable	Purpose	Required	Secret	Default
OPENAI_API_KEY	LLM specification extraction	Yes	Yes	None
UNISPEC_OUTPUT_DIR	Output directory	No	No	./unispec-output
